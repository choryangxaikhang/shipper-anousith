...
import { QUERY_BORROW } from "../../home/apollo";
export default function DetailIBorrow({ _id, onHide }) {
  const [show, setShow] = useState(false);
  const [fetchData, { data: dataBonus, loading }] = useLazyQuery(QUERY_BORROW, {
    ...
    <>
      ...
      >
        <Modal.Header style={{ marginTop: -20 }}>
          <Modal.Title>ລາຍລະອຽດເງິນເບິກລ່ວງໜ້າ</Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-2  mr-4">
          <div className="text-center">{loading && loadingData(25)}</div>
          {dataBonus?.borrowPayroll?.data?.map((data, index) => (
            <>
