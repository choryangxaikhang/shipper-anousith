...
import { EDIT_TYPE_HOUSE } from "./apollo";
import Notiflix, { Loading } from "notiflix";
export default function EditType({ getId, onSuccess }) {
  const [show, setShow] = useState(false);
  const [editProvinces] = useMutation(EDIT_TYPE_HOUSE);
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title className="fs-5">
                <i className="icon-sliders" /> ແກ້ໄຂຂໍ້ມູນປະເພດກິດຈະການ
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <div className="form-group">
                <label>ປະເພດກິດຈະການ (ພາສາລາວ){valiDate()}</label>
                <input
                  type="text"
                  className={
                    errors.title_lao
              ...
              <div className="form-group">
                <label>ປະເພດກິດຈະການ (ພາສາອັງກິດ)</label>
                <input
                  type="text"
